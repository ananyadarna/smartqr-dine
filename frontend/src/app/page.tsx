...
function SubdomainNotFound() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0f1d] flex flex-col items-center justify-center text-center p-6 select-none selection:bg-orange-500 selection:text-white">
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="max-w-md bg-[#0c1224] border border-slate-800 rounded-3xl p-8 shadow-2xl relative overflow-hidden"
      >
        ...
        <h1 className="text-2xl font-black text-white tracking-tight mb-3">Restaurant Portal Not Found</h1>
        <p className="text-slate-400 font-light text-sm leading-relaxed mb-8">
          The requested subdomain is not registered on our platform. Please double-check the URL or contact the restaurant administration.
        </p>

        <a
          href={rootDomain}
          className="w-full py-3.5 px-6 bg-slate-800 hover:bg-slate-700/80 border border-slate-700 text-slate-200 rounded-xl font-bold transition flex items-center justify-center gap-2 text-sm cursor-pointer"
        >
...
function RestaurantWelcomePage({ restaurant }: { restaurant: any }) {
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 relative overflow-hidden flex flex-col justify-between selection:bg-brand-orange selection:text-white">
      {/* Dynamic Background Blob with theme color */}
      <div className="absolute top-0 left-1/4 w-120 h-120 bg-brand-orange/5 rounded-full blur-[100px] pointer-events-none"></div>

      {/* Main content container */}
      <div className="w-full max-w-2xl mx-auto px-4 pt-12 pb-24 flex-1 flex flex-col justify-center">
        ...
        >
          ...
          <div className="relative h-48 md:h-64 bg-slate-900 overflow-hidden">
            {restaurant.banner ? (
              ...
              <div className="w-full h-full bg-linear-to-tr from-brand-navy to-slate-900 flex items-center justify-center">
                <UtensilsCrossed className="w-16 h-16 text-brand-orange/20" />
              </div>
            )}
          ...
          <div className="p-6 md:p-8 relative">
            ...
            <div className="absolute -top-12 left-6 md:left-8 w-24 h-24 rounded-2xl bg-white border border-slate-100 p-1.5 shadow-md overflow-hidden">
              {restaurant.logo ? (
                <img
                  src={restaurant.logo}
                  alt={restaurant.name}
                ...
                <div className="w-full h-full bg-brand-orange flex items-center justify-center text-white rounded-xl">
            ...
            <div className="pt-14 space-y-4">
              ...
              <p className="text-slate-500 font-light text-sm md:text-base leading-relaxed">
                Welcome! Scan the QR code placed on your table to browse our live digital menu, customize items, and place your order directly.
              </p>

              {/* Action Info */}
              <div className="pt-4">
                <div className="inline-flex items-center gap-3 bg-brand-orange/10 border border-brand-orange/20 text-brand-orange px-4 py-3 rounded-2xl text-sm font-semibold max-w-full">
                  <QrCode className="w-5 h-5 shrink-0" />
                  <span>Scan table QR code to start ordering</span>
                </div>
              </div>
            </div>
          </div>
        ...
        >
          {restaurant.phone && (
            <div className="bg-white border border-slate-200/50 rounded-2xl p-5 shadow-sm flex items-start gap-4">
              <div className="w-10 h-10 rounded-xl bg-brand-orange/10 border border-brand-orange/25 flex items-center justify-center text-brand-orange shrink-0">
                <Phone className="w-4.5 h-4.5" />
              </div>
              <div className="overflow-hidden">
                <span className="text-xs text-slate-400 font-bold block uppercase tracking-wider">Phone</span>
                <span className="font-extrabold text-slate-700 text-sm mt-0.5 block">{restaurant.phone}</span>
              </div>
            </div>
          )}
          ...
          {restaurant.email && (
            <div className="bg-white border border-slate-200/50 rounded-2xl p-5 shadow-sm flex items-start gap-4">
              ...
              <div className="overflow-hidden">
                <span className="text-xs text-slate-400 font-bold block uppercase tracking-wider">Email</span>
                <span className="font-extrabold text-slate-700 text-sm mt-0.5 block truncate">{restaurant.email}</span>
              </div>
            </div>
          )}

          {restaurant.address && (
            <div className="bg-white border border-slate-200/50 rounded-2xl p-5 shadow-sm flex items-start gap-4 sm:col-span-2">
              <div className="w-10 h-10 rounded-xl bg-brand-orange/10 border border-brand-orange/25 flex items-center justify-center text-brand-orange shrink-0">
                <MapPin className="w-4.5 h-4.5" />
              </div>
              <div className="overflow-hidden">
                <span className="text-xs text-slate-400 font-bold block uppercase tracking-wider">Location Address</span>
                <span className="font-extrabold text-slate-700 text-sm mt-0.5 block leading-relaxed">{restaurant.address}</span>
              </div>
            </div>
          )}
        </motion.div>
      ...
      {/* Branded White-Label Footer */}
      <footer className="border-t border-slate-200/60 bg-white/50 backdrop-blur-md py-6 px-4 text-center text-slate-400 text-xs shrink-0 z-10 flex flex-col sm:flex-row justify-between items-center gap-4 max-w-2xl mx-auto w-full rounded-t-3xl border-x">
        <div className="flex items-center gap-1.5 font-medium text-slate-500">
          <UtensilsCrossed className="w-3.5 h-3.5 text-slate-400 animate-pulse" />
          <span>Powered by <span className="font-semibold text-slate-650">SmartQR Dine</span></span>
        </div>
        <Link
          href="/auth/login"
          className="flex items-center gap-1.5 text-brand-orange hover:text-brand-orange-hover font-semibold transition"
        >
...
export default function LandingPage() {
  ...
  if (subdomain) {
    ...
    return <RestaurantWelcomePage restaurant={restaurantData} />;
  }
